...
class Bank {
    customer: Customer[] = [];
    account: BankAccount[] = [];
    addCustomer(objCuss: Customer) {
        this.customer.push(objCuss);
    }
    addAccount(objAcc: BankAccount) {
        this.account.push(objAcc);
    }
    transaction(objAcc: BankAccount) {
        let newAmount = this.account.filter(
            (val) => val.accNumber !== objAcc.accNumber
        );
        this.account = [...newAmount, objAcc];
    }
}
let autherName: string = "Huzaifa Abdulrab";
console.log(`>>>>>>>>>>>>>>>>>>>>>>>>${chalk.blueBright(autherName)}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<`);
let bankAccountDetail: string = "This bank has created over 100 accounts, and all account passwords start from 1001."
            ...
            if (!account) {
                ...
                console.log(
                    `Dear ${chalk.green.italic(name?.firstName)} ${chalk.green.italic(
                        name?.lastName
                    )} your Account balance is ${chalk.yellow("$" + account.balance)}`
                );
            }
        }
        // Cash Withdraw
